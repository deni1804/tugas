...
class Booking extends CI_Controller
{
    ...
    public function tambahBooking()
    {
        ...
        //memilih data buku yang untuk dimasukan ketabel temp/keranjang melalui variable $isi
        ...
        //cek jika sudah memasukan 3 buku untuk di booking dalam keranjang
        $tempuser = $this->db->query("select*from temp where id_user ='$userid'")->num_rows();

        //cek jika ada booking buku yang belum diambil
        ...
        //jika buku yang diklik sudah ada di keranjang
        if ($temp > 0) {
        ...
        if ($tempuser == 3) {
            $this->session->set_flashdata('pesan', '<div class="alert alert-danger alert-message" role="alert">Booking Tidak Boleh Lebih dari 3</div>');
            redirect(base_url() . 'home');
        }
        ...
        $this->session->set_flashdata('pesan', '<div class="alert alert-success alert-message" role="alert">Buku berhasil di tambahkan ke keranjang</div>');
        redirect(base_url() . "home");
    }
    ...
    {
        //mengupdate setok dan dibooking di tabel buku saat proses booking di sekesaikan
        ...
        //menyimpan ke table booking dan detail booking, dan mengosongkan tabel temporari
    ...
    public function exportToPdf()
    {
        ...
        $this->dompdf->render();
        $this->dompdf->stream("bukti-booking-$id_user.pdf", array('Attachment' => 0)); //nama pdf yang di hasilkan
